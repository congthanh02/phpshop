<section id="halink_vn_homepage_content" style="margin-top:  18%;">
 	<div class="container">
	<!-- Title -->
        <div class="title-wrapper  woodmart-title-color-default woodmart-title-style-default woodmart-title-width-100 text-center woodmart-title-size-medium ">
            <div class="liner-continer">
            	<span class="left-line"></span>
            	<h4 class="wle-upper woodmart-title-container title  woodmart-font-weight-">Giới Thiệu</h4>
            	<span class="right-line"></span>
            </div>
        </div>
	</div>
</section>
<!-- pages-title-end -->
<!-- about-us-section-start -->
<div class="pages about-us section-padding">
	<div class="container">
		<div class="row">
				<div class="col-lg-6">
					<div class="about-img">
						<img src="public/images/logo_tvd.png" alt=""  />
					</div>
				</div>
				<div class="col-lg-6">
					<div class="about-text">
						<div class="row">
						<h4>Cửa hàng thời trang TD</h4>
						<ol>
						<li>TD là một cửa hàng thời trang chuyên cung cấp các loại sản phẩm về áo, quần, mũ, thắt lưng và giày.</li>
						<li>Luôn cập nhật những sản phẩm mới nhất mỗi ngày, TD mong muốn đem lại cho khách hàng những giá trị tốt nhất về sản phẩm cũng như các dịch vụ tại TD.</li>
						<li>Với không gian mua sắm sang trọng, hiện đại, đội ngũ nhân viên tận tình, chu đáo, cùng với đó là dịch vụ chăm sóc khách hàng vô cùng hoàn hảo, TD hy vọng tất cả khách hàng sẽ có những giây phút shopping thật thoải mái.</li>
						</ol>
						</div>
						<div class="row" style="margin-top: 5px">
						<h4>Cửa hàng chúng tôi với:</h4>
						<ol>
							<li>Các chương trình khuyến mãi hấp dẫn</li>
							<li>Thái độ phục vụ tốt</li>
							<li>Nguồn cung cấp uy tín và ổn định</li>
						</ol>
						</div>
					</div>
				</div>
		</div>
	</div>
</div>
<!-- about-us section end -->
<!-- team-member section start -->
<p class="container" style="font-size: 30px;margin-top: 3%;color: black;">Đội Ngũ Hỗ Trợ</p>
<div class="container">
	<div class="row" style="margin-left:  4%;margin-right:  4%;">
		<div class="col-4">
			<div class="card card_gioithieu" >
				<img src="public/images/tintuc2.jpg" alt="card background" class="card-img">
                <img src="public/images/profile_dien.jpg" alt="profile image" class="profile-img">
                <h1>Nguyễn Minh Diện</h1>
                
                <p class="about">
                    Chúng ta có thể gặp nhiều thất bại nhưng chúng ta không được bị đánh bại.
                </p>
                <a href="#" class="btn btn_gioithieu">Theo dõi</a>
                <ul class="social-media">
                    <a href=""><i class="fab fa-facebook-f"></i></a>
                    <a href=""><i class="fab fa-twitter"></i></a>
                    <a href=""><i class="fab fa-instagram"></i></a>
                </ul>
			</div>

    	</div>	
    	<div class="col-4">
			<div class="card card_gioithieu" >
				<img src="public/images/tintuc2.jpg" alt="card background" class="card-img">
                <img src="public/images/profile_thu.jpg" alt="profile image" class="profile-img">
                <h1>Đặng Ngọc Thư</h1>
                
                <p class="about">
                    Nếu bạn muốn trở nên kỳ quặc khác người, hãy tự tin khi làm điều đó.
                </p>
                <a href="#" class="btn btn_gioithieu">Theo dõi</a>
                <ul class="social-media">
                    <a href=""><i class="fab fa-facebook-f"></i></a>
                    <a href=""><i class="fab fa-twitter"></i></a>
                    <a href=""><i class="fab fa-instagram"></i></a>
                </ul>
			</div>

    	</div>	
    	<div class="col-4">
			<div class="card card_gioithieu" >
				<img src="public/images/tintuc2.jpg" alt="card background" class="card-img">
                <img src="public/images/tintuc3.jpg" alt="profile image" class="profile-img">
                <h1>Thị Vi</h1>
                
                <p class="about">
                    Nỗi đau bạn cảm thấy hôm nay là sức mạnh bạn sẽ cảm thấy vào ngày mai.
                </p>
                <a href="#" class="btn btn_gioithieu">Theo dõi</a>
                <ul class="social-media">
                    <a href=""><i class="fab fa-facebook-f"></i></a>
                    <a href=""><i class="fab fa-twitter"></i></a>
                    <a href=""><i class="fab fa-instagram"></i></a>
                </ul>
			</div>

    	</div>			
    </div>
</div>

<!-- team-member section end -->
